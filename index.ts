...
const zipUrl = "https://github.com/apple/device-management/archive/refs/heads/release.zip";
// download zip file
...
// get the yaml files
const yamlFiles = Object.entries(zip.files()).filter(([path]) => path.startsWith("device-management-release/mdm/commands") && path.endsWith(".yaml"));
...
function generateDict(zod: boolean, parsed: { key: string, type: string, presence?: string, subkeys?: any }[]) {
    let type = zod ? "z.object({\n" : "{\n";
    for (const key of parsed) {
        if (key.type === "<dictionary>") {
            type += checkKey(key.key) + ": " + generateDict(zod, key.subkeys) + checkOptional(key, zod) + ",\n";
            continue;
        }
        if (key.type === "<array>") {
            type += checkKey(key.key) + ": " + generateArray(zod, key) + checkOptional(key, zod) + ",\n";
            continue;
        }
        assert(map[key.type], `Type ${key.type} not found`);
        type += checkKey(key.key) + ": " + map[key.type](zod) + checkOptional(key, zod) + ",\n";
    }
    type += zod ? "})" : "}";
    return type;
}
...
function generateArray(zod: boolean, parsed: any) {
    ...
    if (key.type === "<dictionary>") {
        type += "\n" + generateDict(zod, key.subkeys) + checkOptional(key, zod) + "\n";
    } else {
        assert(map[key.type], `Type ${key.type} not found`);
...
for (const [_, file] of yamlFiles) {
    ...
    if (!parsed.responsekeys) {
        console.log(parsed.payload.requesttype + " has no response skipping...");
        continue;
    }
    const name = parsed.payload.requesttype + ".ts";
    const zod = `import { z } from "https://deno.land/x/zod/mod.ts";\nexport const ${parsed.payload.requesttype} = ${generateDict(true, parsed.responsekeys)};`;
    Deno.writeTextFileSync("generated/zod/" + name, await fmt.format(zod));

    const ts = `export type ${parsed.payload.requesttype} = ${generateDict(false, parsed.responsekeys)};`;
